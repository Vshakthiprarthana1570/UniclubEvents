...
def read_note():
    ...
    command = "SELECT note_id, timestamp, note FROM notes " 
    _c.execute(command)
    result = _c.fetchall()
...
def image_upload_record(uid, owner, image_name, timestamp,description):
    _conn = sqlite3.connect(image_db_file_location)
    _c = _conn.cursor()

    _c.execute("INSERT INTO images VALUES (?, ?, ?, ?,?)", (uid, owner, image_name, timestamp,description))
...
def list_images():
    ...
    #command = "SELECT uid, timestamp, name FROM images WHERE owner = '{0}'".format(owner)
    command = "SELECT uid,timestamp,name,description FROM images"
    _c.execute(command)
    result = _c.fetchall()
